import dbData from './db/poker_book.json';

// Caché del mapa maestro para la navegación app
let cachedMap = null;

export const generatePokerNavigationTree = () => {
  let idCounter = 1;

  // Procesador recursivo de nodos del JSON
  const processNode = (node, currentLevel) => {
    let children = node.chapters || node.sections || node.subsections || [];

    return {
      id: node.id || `poker-node-${idCounter++}`,
      title: node.title,
      type: 'theory',
      level: currentLevel,
      content: node.content || "",
      subsections: children.map(sub => processNode(sub, currentLevel + 1))
    };
  };

  // Distribución del libro
  let children = dbData.chapters || dbData.sections || dbData.subsections || [];
  return children.map(n => processNode(n, n.level !== undefined ? n.level : 1));
};
...
export const PokerAPI = {
   ...
   */
  getNavigationMap: () => {
    cachedMap = generatePokerNavigationTree();
    return cachedMap;
  },
   ...
   */
  getStats: () => {
    const map = PokerAPI.getNavigationMap();
    let totalNodes = 0;
