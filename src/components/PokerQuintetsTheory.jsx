...
import { pokerQuintetsBlocks } from '../data/poker_quintets_theory';
import { Waypoints, Network, Briefcase, AlertTriangle, ChevronRight, LayoutPanelTop, Target } from 'lucide-react';
...
export default function PokerQuintetsTheory() {
  const [activeTab, setActiveTab] = useState(pokerQuintetsBlocks[0].id);

  const activeBlock = pokerQuintetsBlocks.find(b => b.id === activeTab) || pokerQuintetsBlocks[0];
      ...
      <div className="w-full md:w-80 flex-shrink-0 bg-black border-r border-red-900/30 flex flex-col h-full md:h-auto overflow-y-auto custom-scrollbar">
        <div className="p-6 border-b border-red-900/30 sticky top-0 bg-black/95 backdrop-blur z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-2xl font-serif text-red-500 leading-tight">Módulo de<br/>5 Cartas</h2>
          </div>
          <p className="text-sm text-gray-400 mt-2">
            Evolución Direccional: El Pivote Soberano y pares espejados.
          </p>
        </div>
        
        <nav className="p-4 flex flex-col gap-2">
          {pokerQuintetsBlocks.map((block) => {
                    ...
                    <div className="space-y-6">
                      <div>
                        <div className="text-xs uppercase tracking-widest text-red-500/60 mb-3 font-bold flex items-center gap-2">
                          <Network className="w-3 h-3" /> Desglose Analítico
                        </div>
                      ...
                      <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                        <div className="text-xs uppercase tracking-widest text-gray-400 mb-3 font-bold flex items-center gap-2">
                          <Target className="w-3 h-3" /> Alteración / Dinámica Relacional
                        </div>
