...
import { pokerNineCardsBlocks } from '../data/poker_ninecards_theory';
import { Grid3X3, Target, HeartCrack, AlertTriangle, ChevronRight, Hash, Network } from 'lucide-react';
...
export default function PokerNineCardsTheory() {
  const [activeTab, setActiveTab] = useState(pokerNineCardsBlocks[0].id);

  const activeBlock = pokerNineCardsBlocks.find(b => b.id === activeTab) || pokerNineCardsBlocks[0];
  const ActiveIcon = iconMap[activeBlock.icon] || Hash;
      ...
      <div className="w-full md:w-80 flex-shrink-0 bg-black border-r border-red-900/30 flex flex-col h-full md:h-auto overflow-y-auto custom-scrollbar">
        <div className="p-6 border-b border-red-900/30 sticky top-0 bg-black/95 backdrop-blur z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-2xl font-serif text-red-500 leading-tight">Módulo<br/>9 Cartas</h2>
          </div>
          <p className="text-sm text-gray-400 mt-2">
            La Matriz Compleja: Cuadrado Mágico y planos dimensionales.
          </p>
        </div>
        
        <nav className="p-4 flex flex-col gap-2">
          {pokerNineCardsBlocks.map((block) => {
            const Icon = iconMap[block.icon] || Hash;
            ...
            return (
              ...
              >
                ...
                {isActive && <ChevronRight className="w-4 h-4 text-red-500 ml-auto flex-shrink-0" />}
              </button>
            )
          })}
        </nav>
              ...
              {activeBlock.examples.map((ej, idx) => (
                <div key={idx} className="bg-[#0f0000] border border-red-900/30 rounded-2xl p-6 md:p-8 hover:border-red-500/50 transition-colors group">
                  ...
                  <div className="flex flex-col gap-6 mb-8 border-b border-red-900/30 pb-6">
                    ...
                    {/* Visualización de la Matriz 3x3 */}
                    {ej.cartas && ej.cartas.length === 9 ? (
                      ...
                      <div className="bg-black/80 rounded-xl p-6 border border-white/5 flex justify-center text-center">
                         <span className="text-gray-400 italic">No hay grid de cartas definido para este bloque visual.</span>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <div className="text-xs uppercase tracking-widest text-red-500/60 mb-3 font-bold flex items-center gap-2">
                          <Network className="w-3 h-3" /> Análisis Dimensional
                        </div>
                      ...
                      <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                        <div className="text-xs uppercase tracking-widest text-gray-400 mb-3 font-bold flex items-center gap-2">
                          <Target className="w-3 h-3" /> Cruces y Diagonales
                        </div>
